import os
from flask import Flask, request, make_response, render_template, url_for, session, abort
...
app.queue_interface = ProcessInterfaceFactory.create_interface(os.environ.get('QUEUE_TOOL'))
...
def redirect():
    if session.get('list_id', None) is None:
        ...
        app.queue_interface.process(session['user_id'], session['list_id'], auth.access_token, auth.access_token_secret)
